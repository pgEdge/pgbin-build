...
function getPGVersion {
	...
	pgShortVersion=`echo $pgFullVersion | cut -f1 -d'.'`
	echo "pgShortVersion = $pgShortVersion and pgFullVersion = $pgFullVersion"
}
...
while true; do
  case "$1" in
    ...
    --build-spock50 ) buildSpock50=true; Source=$2; shift; shift ;;
    --build-spock60 ) buildSpock60=true; Source=$2; shift; shift ;;
...
if [[ $buildSpock50 == "true" ]]; then
	buildComp spock50  "" "$spock50V" "$spockBld50V" "$Source"
fi

if [[ $buildSpock60 == "true" ]]; then
	buildComp spock60  "" "$spock60V" "$spockBld60V" "$Source"
fi
